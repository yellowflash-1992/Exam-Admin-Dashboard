...
export default async function NabtebPage() {
  const dashboard = await getDashboardData("NABTEB");

  const examination = examinations.find((exam) => exam.name === "NABTEB");

  if (!examination) {
    throw new Error("NABTEB examination configuration not found");
  }
