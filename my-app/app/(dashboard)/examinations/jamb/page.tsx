...
export default async function JambPage() {
  const dashboard = await getDashboardData("JAMB");

  const examination = examinations.find((exam) => exam.name === "JAMB");

  if (!examination) {
    throw new Error("JAMB examination configuration not found");
  }
